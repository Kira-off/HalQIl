...
export default function AdminDisputesPage() {
  ...
  const { data: disputes = [], isLoading, isError } = useQuery<Order[]>({
    ...
    <div className="max-w-6xl mx-auto space-y-6 py-4 sm:py-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl sm:text-2xl font-black text-gray-900 leading-tight">
            Nizolarni Hal Qilish Paneli
          </h1>
          <p className="text-xs text-gray-400 font-semibold mt-0.5">
            Mijozlar va provayderlar o&apos;rtasidagi muammoli buyurtmalarni tekshirish va yakuniy qaror chiqarish
          </p>
        </div>
      </div>
      ...
      {isLoading ? (
        <div className="bg-white border border-gray-150 rounded-3xl p-12 flex flex-col items-center justify-center space-y-3 shadow-sm">
          ...
          <p className="text-xs text-gray-500 font-bold">Nizolar yuklanmoqda...</p>
        </div>
      ) : isError ? (
        <div className="bg-rose-50 border border-rose-150 text-rose-700 p-6 rounded-3xl text-sm font-semibold flex items-center gap-3">
          ...
          Nizolar ro&apos;yxatini yuklashda xatolik yuz berdi.
        </div>
      ) : disputes.length === 0 ? (
        <div className="bg-white border border-gray-150 rounded-3xl p-12 text-center text-sm font-semibold text-gray-400 shadow-sm">
          Ayni vaqtda faol nizolar mavjud emas. Barchasi tinch!
        </div>
      ) : (
        <div className="grid grid-cols-1 gap-6 animate-fade-in">
          {disputes.map((order) => (
            <DisputeResolver key={order.id} order={order} />
          ))}
        </div>
      )}
    </div>
